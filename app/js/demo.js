...
function onChange($el, cb) {
  var last = $el.val()  $el.on('input', function() {
    var val = $el.val()    var oldVal    if (last !== val) {
      oldVal = last
      last = val
...
function prepareDownload(string, pattern) {
  ...
  var ctx = canvas.getContext('2d')  var img = new Image()  img.onload = function() {
...
var changeEvent = onChange($('#string'), function(val) {
  var pattern = GeoPattern.generate(val)  var bg = next()  $('#bg-' + bg.next)
...
$(function() {
  ...
  $('#change').on('click', function() {
    var buf = new Buffer(str.replace(/^data:image\/png;base64,/, ''), 'base64')    var filename = os.homedir() + '/Pictures/Patterns/' + saveButton.download    fs.writeFile(filename, buf, function(err) {
      console.log(err);
      console.log('done');
      if (!err) {
